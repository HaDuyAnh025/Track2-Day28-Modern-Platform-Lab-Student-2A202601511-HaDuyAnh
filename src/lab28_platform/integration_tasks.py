...
def event_headers(
    ...
    headers.append(("idempotency-key", idempotency_key.encode()))
    return headers
...
def readiness_status(probes: Iterable[dict[str, Any]]) -> str:
    ...
    statuses = list(probes)
    if any(not probe["ready"] and probe["mandatory"] for probe in statuses):
        return "not_ready"
    if any(not probe["ready"] for probe in statuses):
        return "degraded"
    return "ready"
